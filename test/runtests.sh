...
java -classpath ../bin hohuan.addressbook.AddressBook 'directoryThatDoesNotExist/valid.filename' < /dev/null >> actual.txt
# (invalid) no parent directory, invalid filename with dot on first character
java -classpath ../bin hohuan.addressbook.AddressBook '.noFilename' < /dev/null >> actual.txt
# (invalid) valid parent directory, non regular file
mkdir -p data/notRegularFile.txt
java -classpath ../bin hohuan.addressbook.AddressBook 'data/notRegularFile.txt' < /dev/null >> actual.txt
# (valid) valid parent directory, valid filename with extension.
touch data/valid.filename
java -classpath ../bin hohuan.addressbook.AddressBook 'data/valid.filename' < exitinput.txt >> actual.txt
# run the program, feed commands from input.txt file and redirect the output to the actual.txt
java -classpath ../bin hohuan.addressbook.AddressBook < input.txt >> actual.txt
